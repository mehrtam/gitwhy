...
"""
import json, os, random, subprocess, sys, time, importlib.util, shutil, stat
...
def gen_history(n_sessions, files_pool, commits_target, bloat_every=6, bloat_mult=1):
    ...
    tag = f"s{n_sessions}"
    claude_dir = WS / tag / "claude"; proj = WS / tag / "proj"
    cproj = claude_dir / "projects" / "x"; cproj.mkdir(parents=True, exist_ok=True)
    proj.mkdir(parents=True, exist_ok=True)
    def git(*a, env=None):
        subprocess.run(["git", "-C", str(proj), *a], capture_output=True, env=env)
    git("init", "-q", "."); git("config", "user.email", "b@b.com"); git("config", "user.name", "B")
    ...
    for i in range(n_sessions):
        ...
        for r in range(rounds):
            ...
            fs = random.sample(files_pool, random.randint(1, 3)); session_files += fs
            ...
            if commits_made < commits_target and (r == rounds - 1 or random.random() < 0.35):
                ...
                for f in set(fs):
                    p = proj / f; p.parent.mkdir(parents=True, exist_ok=True)
                    p.write_text(f"# {random.random()}\n")
                genv["GIT_AUTHOR_DATE"] = genv["GIT_COMMITTER_DATE"] = t.isoformat()
                git("add", "."); git("commit", "-qm", random.choice(PROMPTS)[:60], env=genv)
                commits_made += 1
...
def run_scale(gw, name, n_sessions, n_files, commits_target, bloat_every=6, bloat_mult=1):
    ...
    claude_dir, proj, commits = gen_history(n_sessions, files_pool, commits_target, bloat_every, bloat_mult)
    size_mb = mb(claude_dir)
    print(f"   generated in {time.perf_counter()-g0:.1f}s · {size_mb:.1f} MB transcripts · {commits} commits")
    ...
    spec = importlib.util.spec_from_file_location(f"gitwhy_{name}", HERE / "gitwhy.py")
    ...
    for s in sessions: gw.relativize(s, proj)
    ...
    raw_tokens = int(raw_chars * (sample_tokens / max(1, len(sample[:400_000]))))
...
def main():
    for f in ("gitwhy.py", "report_template.html"):
        if not (HERE / f).exists():
            print(f"[!] {f} must be in the same folder."); sys.exit(1)
    wipe(WS); WS.mkdir(parents=True, exist_ok=True)
