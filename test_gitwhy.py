...
def wipe(path: Path):
    """OneDrive-proof rmtree: clear read-only bits, retry, fall back to renaming."""
    import shutil
    import stat
    import time
    if not path.exists():
...
def main():
    if not (HERE / "gitwhy.py").exists() or not (HERE / "report_template.html").exists():
        print("[!] Put test_gitwhy.py in the SAME folder as gitwhy.py and "
              "report_template.html, then run it from there.")
        sys.exit(1)
    wipe(WS)
    ...
    xdir = codex_dir / "sessions" / "2026" / "07" / "26"
    ...
    check("archiver saved transcripts from BOTH agents",
          len(archived) == 3, f"got {len(archived)}")

    print("\n== parsing & linking ==")
    sessions = gw.find_sessions(PROJ)
    agents = sorted(s.get("agent") for s in sessions)
    check("both sessions found (claude + codex)",
          agents == ["claude", "codex"], str(agents))
    check("resume duplicate deduped", len(sessions)
          == 2, f"{len(sessions)} sessions")
    ...
    check("claude: noise (401) filtered from stories/excerpts",
          cl and all("401" not in (st.get("reason") or "")
                     for st in cl["stories"])
    ...
    check("both sessions linked to their commits",
          len(links) >= 2, f"{len(links)} links")
    check("commit-inside-session evidence present",
          any(l["inside"] for l in links))
    ...
    if md_out.exists():
        ...
        check("GITWHY.md covers both files",
              "train.py" in md and "eval.py" in md)
        check("GITWHY.md is token-cheap (< 2000 chars here)",
              len(md) < 2000, f"{len(md)} chars")
    if html_out.exists():
